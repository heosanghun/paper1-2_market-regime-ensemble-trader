...
# 저장 경로
save_dir = r"D:/drl-candlesticks-trader-main1/captures/1w"
...
window_days = 730  # 2년 단위로 변경
cur = start_date

while cur + timedelta(days=window_days) <= end_date:
    s = cur.strftime("%Y-%m-%d")
    e = (cur + timedelta(days=window_days)).strftime("%Y-%m-%d")
    klines = client.get_historical_klines("BTCUSDT", Client.KLINE_INTERVAL_1WEEK, s, e)
